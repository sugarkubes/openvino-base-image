...
RUN apt-get update && apt-get install -y --no-install-recommends \
            ...
            wget \
            unzip
WORKDIR /sugar-server

COPY requirements.txt /sugar-server/
RUN virtualenv -p python3 .venv && \
    . .venv/bin/activate && pip3 install -r requirements.txt

COPY start_server.sh setup.py requirements.txt version /sugar-server/
COPY server /sugar-server/server
